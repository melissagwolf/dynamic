#' @title Dynamic fit index (DFI) cutoffs for continuous, non-normal multi-factor CFA models with (possible) missing data
#'
#' @description This function generates DFI cutoffs for multi-factor CFA models that treats items as continuous and non-normal with
...
#'
#' \donttest{nnorHB(fit, data=Example)}
#'
...
#'  F1~~.339*F2"
#'
#' \donttest{nnorHB(model=manual_model,data=Example,n=500,manual=TRUE)}
#'

nnorHB <- function(model,data,n=NULL,plot=FALSE,manual=FALSE,estimator="MLR",reps=500){
  ...
  #Run simulation
  tryCatch(multi_df_nnor(model9,data,n,estimator, reps),
           error=function(err3){
  ...
  results <- multi_df_nnor(model9,data,n,estimator, reps)
